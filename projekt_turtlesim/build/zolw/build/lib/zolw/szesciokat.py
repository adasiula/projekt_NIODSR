...
class DrawHexagonNode(Node):
    ...
    def pose_callback(self,pose:Pose):
        ...
        if(self.pobranie_pozycji_szesciokata):
            ...
            self.poz_x2=self.poz_x1+math.cos(pose.theta+self.rel_kat_w)*self.dystans
            self.poz_y2=self.poz_y1+math.sin(pose.theta+self.rel_kat_w)*self.dystans
            self.poz_x3=self.poz_x2+math.cos(pose.theta+2*self.rel_kat_w)*self.dystans
            self.poz_y3=self.poz_y2+math.sin(pose.theta+2*self.rel_kat_w)*self.dystans
            self.poz_x4=self.poz_x3+math.cos(pose.theta+3*self.rel_kat_w)*self.dystans
            self.poz_y4=self.poz_y3+math.sin(pose.theta+3*self.rel_kat_w)*self.dystans
            self.poz_x5=self.poz_x4+math.cos(pose.theta+4*self.rel_kat_w)*self.dystans
            self.poz_y5=self.poz_y4+math.sin(pose.theta+4*self.rel_kat_w)*self.dystans
        ...
        if(self.wierzcholek):
            match self.w:
                ...
                case 4:
                    self.x_goal=self.poz_x5
                    self.y_goal=self.poz_y5
                case 5:
...
def main(args=None):
    ...
    node_hexagon = DrawHexagonNode()
    node_hexagon.dystans=float(input('dlugosc boku?\n'))
    rclpy.spin(node_hexagon)
    rclpy.shutdown()