...
class DrawSquareNode(Node):
    ...
    def pose_callback(self,pose:Pose):
        ...
        # print(self.obr)
        # print("sin")
        # print(math.sin(pose.theta))
        # print("cos")
        # print(math.cos(pose.theta))
        ...
        if(self.pobranie_pozycji_kwadratu):
            ...
            self.poz_x1=self.poz_x0+math.cos(pose.theta)*self.distance
            self.poz_y1=self.poz_y0+math.sin(pose.theta)*self.distance
            self.poz_x2=self.poz_x1+math.cos(pose.theta+PI/2)*self.distance
            self.poz_y2=self.poz_y1+math.sin(pose.theta+PI/2)*self.distance
            self.poz_x3=self.poz_x2+math.cos(pose.theta+PI)*self.distance
            self.poz_y3=self.poz_y2+math.sin(pose.theta+PI)*self.distance
